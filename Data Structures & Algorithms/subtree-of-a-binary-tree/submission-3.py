...
class Solution:   
    def isSubtree(self, root: Optional[TreeNode], subRoot: Optional[TreeNode]) -> bool:
        if not root: # If tree is empty, no subtree can exist in it 
            return False 
        if self.isSameTree(root, subRoot): # Checking if current node is start of subRoot (Even if both are null)
            return True
        else: # Check if any of the children is the start of the subRoot
